...
class CreateNewUser implements CreatesNewUsers
{
    ...
    protected function validate(array $input): void
    {
        ...
        Validator::make($input, [
            ...
            'document_number' => [Rule::requiredIf($hasDocumentNumber), 'nullable', 'string', 'max:30', Rule::unique(User::class)],
            'date_of_birth' => ['required', 'date', 'before:today'],
            'gender' => ['required', Rule::enum(Gender::class)],
            'password' => $this->passwordRules(),

            'country_id' => ['required', 'exists:countries,id'],
            'voivodeship_id' => [Rule::requiredIf($isPoland), 'nullable', 'exists:voivodeships,id'],
            'state' => [Rule::requiredIf(! $isPoland), 'nullable', 'string', 'max:255'],
            ...
            'm_country_id' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'exists:countries,id'],
            'm_voivodeship_id' => [Rule::requiredIf($hasDifferentMailingAddress && $isPoland), 'nullable', 'exists:voivodeships,id'],
            'm_state' => [Rule::requiredIf($hasDifferentMailingAddress && ! $isPoland), 'nullable', 'string', 'max:255'],
            'm_post_code' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'string', 'max:10'],
            'm_city' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'string', 'max:255'],
            'm_street' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'string', 'max:255'],
            'm_house_number' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'string', 'max:10'],
            'm_apartment_number' => ['nullable', 'string', 'max:10'],
            'm_post_office' => [Rule::requiredIf($hasDifferentMailingAddress), 'nullable', 'string', 'max:255'],
        ])->validate();
    }
}
